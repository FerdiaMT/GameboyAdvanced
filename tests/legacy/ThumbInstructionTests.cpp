...
const char* DebuggerCPU::checkConditional(uint8_t cond)
{
...
std::string DebuggerCPU::thumbToStr(CPU::thumbInstr& instr)
{
	...
	if (instr.rd != 0) ss << " Rd=R" << static_cast<int>(instr.rd);
	if (instr.rs != 0) ss << " Rs=R" << static_cast<int>(instr.rs);
	if (instr.rn != 0) ss << " Rn=R" << static_cast<int>(instr.rn);
...
bool testBEQ_Backward(CPU& cpu)
{
    ...
    instr.type = CPU::thumbOperation::THUMB_B_COND;
    instr.cond = 0x0;  // EQ
    instr.imm = -4 * 2;  // Already shifted: -8 bytes
    cpu.opT_B_COND(instr);
    // PC = 0x100 + 4 + (-8) = 0xFC
...
bool testBranch_MaxBackward(CPU& cpu)
{
    ...
    instr.type = CPU::thumbOperation::THUMB_B_COND;
    instr.cond = 0x0;  // EQ
    instr.imm = -128 * 2;  // Already shifted: -256 bytes
    cpu.opT_B_COND(instr);
    // PC = 0x100 + 4 + (-256) = 0x04
...
bool DebuggerCPU::runAllThumbTests(CPU& cpu)
{
    int passCount = 0;
    int failCount = 0;
